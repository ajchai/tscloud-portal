<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
	...
	<table width="670px" height="100%" border="0" cellpadding="5" cellspacing="0">  
	      <tr> 
	      		<td width="140px" height="60px" align="right" style="font-size:14px">推荐配置：</td>
	      		<td> 
	      ...
	      <tr>
	      		<td align="right" valign="top" style="font-size:14px" >SharePoint地址：</td> 
				<td>
	      ...
	      <tr>
			  	...
			  	<td>
			  		<div id="div_diy" style="width:100%;height:100%;margin:3px;">
			  			<font style="font-size:12px">请输入地址：</font><input type="text" id="domain" maxLength="20" data-isok="no" onchange="checkDiyDomainInput(this)" style="font-size:12px;width:150px;height:25px;"  class="k-textbox" value="" />
			  		...
			  		<div id="div_default" style="width:100%;height:100%;display:none">
			  			<font style="font-size:12px">请输入地址：</font><input type="text" id="system_default_domain" maxLength="20" data-isok="no" style="font-size:12px;width:80px;" onchange="checkDefaultDomainInput(this)" class="k-textbox" value="" /><font id="system_default_domain2" style="font-size:12px;">.hp-service365.com</font>
			  		</div>
			  	</td>
	      	
	      </tr>
		  <tr>
			<td height="80px" align="right" valign="top" style="font-size:14px;padding-top:10px">最大存储容量：</td>
			<td> 
			    <div style="width:510px;position:relative;">
				 ...
				 <div style="position:relative;width:410px;padding:0px;border:1px solid white;">
					...
					<span style="position:absolute;left:60px;top:-5px;font-size:10px;">100GB</span>
					<span style="position:absolute;left:178px;top:-5px;font-size:10px;">250GB</span>
					<span style="position:absolute;right:0px;top:-5px;font-size:10px;">500GB</span>
					<p style="margin-top:20px;margin-bottom:0px;font-size:12px;">当前选择：<span id="txt" style="color:#D7410B;font-size:12px;">5</span>GB(以2GB为步长增加)</p>
				</div>
			</td> 
		  </tr>
		  <tr>
...
<script type="text/javascript">
	...
	// 得到服务的配置模板项
	Core.AjaxRequest({
				url : ws_url + "/rest/service/templatelist/115",
				type: "get",
				callback : function (data) {
	...
	function selectedSpec(obj,data){ 
		...
		for(var i=0;i < data.specifications.length;i++){
			 if(data.specifications[i].name == "SHAREPOINT_VOLUME"){
				// 判断数据是否为数据盘
	...
	$("#save_submit").click(function(){
				...
				serviceSpec.SHAREPOINT_VOLUME = eachMailVolume;
				var specJson = JSON.stringify(serviceSpec);
				
				if(billing == "Year"){
					var buyTime = $("#billingDateYear").val();
					var table = $("<input type='hidden' id='specJson"+i+"' value='"+specJson+"'/><input type='hidden' id='serviceName"+i+"' value='SharePoint服务'/><input type='hidden' id='serviceSid"+i+"' value='115'/><input type='hidden' id='tempSid"+i+"' value='"+tempSid+"'/><table width='100%' height='100%' border='0' cellspacing='0' cellpadding='3'><tr><td width='20%' align='right'>配置:</td><td width='80%' style='padding-right:18px;' align='left'><span id='spec"+i+"'>最大存储容量:"+eachMailVolume+"GB、SharePoint地址:"+domain+"</span></td></tr><tr><td align='right'>数量:</td><td  align='left'><input id='count"+i+"' type='number' style='width:100px;font-size:12px' value='"+count+"' min='1' max='2' /></td></tr><tr><td align='right'>时长:</td><td  align='left'><div style='width:100%;height:100%;position:relative;'><select id='style_time"+i+"' size='1' style='width:60px;font-size:12px;' onchange='switchTime(this)' placeholder='Select size...'><option value='Year'>按年</option><option value='Month'>按月</option></select>&nbsp;&nbsp; <div id='month"+i+"' style='position:absolute;top:0px;left:70px;'><select id='time"+i+"' size='1' style='width:70px;font-size:12px;' placeholder='Select size...'><option value='1年'>1年</option><option value='2年'>2年</option><option value='3年'>3年</option></select></div></div></td></tr></table>");
				}else{ 
					var buyTime = $("#billingDateMonth").val();
					var table = $("<input type='hidden' id='specJson"+i+"' value='"+specJson+"'/><input type='hidden' id='serviceName"+i+"' value='SharePoint服务'/><input type='hidden' id='serviceSid"+i+"' value='115'/><input type='hidden' id='tempSid"+i+"' value='"+tempSid+"'/><table width='100%' height='100%' border='0' cellspacing='0' cellpadding='3'><tr><td width='20%' align='right'>配置:</td><td width='80%' style='padding-right:18px;' align='left'><span id='spec"+i+"'>最大存储容量:"+eachMailVolume+"GB、SharePoint地址:"+domain+"</span></td></tr><tr><td align='right'>数量:</td><td  align='left'><input id='count"+i+"' type='number' style='width:100px;font-size:12px' value='"+count+"' min='1' max='2' /></td></tr><tr><td align='right'>时长:</td><td  align='left'><div style='width:100%;height:100%;position:relative;'><select id='style_time"+i+"' size='1' style='width:60px;font-size:12px;' onchange='switchTime(this)' placeholder='Select size...'><option value='Year'>按年</option><option value='Month'>按月</option></select>&nbsp;&nbsp; <div id='month"+i+"' style='position:absolute;top:0px;left:70px;'><select id='time"+i+"' size='1' style='width:60px;font-size:12px;' placeholder='Select size...'><option value='1月'>1个月</option><option value='2月'>2个月</option><option value='3月'>3个月</option><option value='4月'>4个月</option><option value='5月'>5个月</option><option value='6月'>6个月</option><option value='7月'>7个月</option><option value='8月'>8个月</option><option value='9月'>9个月</option></select></div></div></td></tr></table>");
				}
	...
	function checkRepeatOrder(){
		var isok = false;
		Core.AjaxRequest({
			url : ws_url + "/rest/order/checkRepeatOrder?serviceSid="+115,
			type : "GET",
			async : false,
			callback : function (data) {
				isok = true;
